...
public class OldworldTeleporter implements ITeleporter {
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("oldworld_portal", Vec3i::compareTo, 300);
	public static PoiType poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegistryEvent.Register<PoiType> event) {
		poi = new PoiType("oldworld_portal",
				com.google.common.collect.Sets
						.newHashSet(ImmutableSet.copyOf(HaegrilontestModBlocks.OLDWORLD_PORTAL.get().getStateDefinition().getPossibleStates())),
				0, 1).setRegistryName("oldworld_portal");
	...
	public OldworldTeleporter(ServerLevel worldServer, BlockPos entityEnterPos) {
				...
				for (int j3 = 0; j3 < 2; ++j3) {
					for (int k3 = -1; k3 < 3; ++k3) {
						BlockState blockstate1 = k3 < 0
								? HaegrilontestModBlocks.POWDERSACK.get().defaultBlockState()
		...
		for (int l1 = -1; l1 < 3; ++l1) {
			for (int j2 = -1; j2 < 4; ++j2) {
				if (l1 == -1 || l1 == 2 || j2 == -1 || j2 == 3) {
					...
					this.level.setBlock(blockpos$mutableblockpos, HaegrilontestModBlocks.POWDERSACK.get().defaultBlockState(), 3);
				}
			}
		}
		BlockState blockstate = HaegrilontestModBlocks.OLDWORLD_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
	...
	private PortalInfo getPortalInfo(Entity entity, ServerLevel server) {
		...
		return this.getExitPortal(entity, blockpos1, worldborder).map(repositioner -> {
			...
			if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
				...
				vector3d = OldworldPortalShape.getRelativePosition(teleportationrepositioner$result, direction$axis, entity.position(),
						entity.getDimensions(entity.getPose()));
			} else {
				direction$axis = Direction.Axis.X;
				vector3d = new Vec3(0.5, 0, 0);
			}
			return OldworldPortalShape.createPortalInfo(server, repositioner, direction$axis, vector3d, entity.getDimensions(entity.getPose()),
